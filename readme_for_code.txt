...
5. Read the top 20 words with largest PageRank centrality in the order of their importance by ProPre.exe in the PreProcess folder as follows.
    ProPre filename K 1
...
1. Sampling %s short texts from the original corpus using Method D implemented as sampling.exe in the PreProcess folder as follows.
    sampling filename s
...
7. Read the top 20 words with largest PageRank centrality in the order of their importance  by ProPre.exe in the PreProcess folder as follows.
    ProPre filename K 1
    where K is the number of clusters, 1 means read top words.

Notice:
1. ProPre.exe, sampling.exe and mapping.exe are all implemented in C Language, their source codes are in the PreProcess folder.
2. This code is not optimized.
3. The experimental results for replications on seven testing corpora (Title, Tweet, Tweet_10, Tset, Tset_50, SMS, Q&A) are included in the results folder.
4. The testing data sets are all included in the PreProcess folder. For anyone who use these data sets, please cite their original references.
