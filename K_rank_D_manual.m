...
ND=size(A,1);%the number of nodes
...
if ~auto
    ...
    for i=1:ND
      if ( (N_inf(i)>rhomin) && (delta(i)>deltamin))
         NCLUST=NCLUST+1;
         cl(i)=NCLUST; % assign cluster labels for cluster centers
...
switch assignment
    case 1
      disp('1:science');
      if ~auto
        for i=1:ND% start from the node with largest density
          if (cl(ordN(i))==-1)
...
switch assignment
    case 4
        ...
        q(2,:)=cl2;
        %qq2=criterion(q');
        qq2=criterion(U2);%each line for each cluster
