...
ND=size(A,1);%number of nodes
new_matrix=adj2vec(A); %signal propogation transformation
...
% % otherwise, the pagerank value is strong enough
 ...
 delta=zeros(1,ND);
 %maxd=max(max(dis));
 [N_sorted,ordN]=sort(N_inf,'descend');%ranking
 delta(ordN(1))=-1.;
 nneigh(ordN(1))=0;
for ii=2:ND   
    list=ordN(1:ii-1);    
...
delta(ordN(1))=max(delta(:)); %cluster-dp's author provided
...
if ~auto
    ...
    for i=1:ND
      if ( (N_inf(i)>rhomin) && (delta(i)>deltamin))
         NCLUST=NCLUST+1;
         cl(i)=NCLUST; % assign cluser labels
...
switch assignment
    case 1
      disp('1:science');
      if ~auto
        for i=1:ND% assign cluser labels for the nodes with large density
          if (cl(ordN(i))==-1)
...
switch assignment
    case 4
        ...
        q(2,:)=cl2;
        %qq2=criterion(q');
        qq2=criterion(U2);%one line, one cluster   
        z2=U2(:,2);
    otherwise
...
%     %result(size(result,1)+1,:)=[assignment,accuracy(qq2,cc),NMI(qq2,cc),evalClustering(z2,realz),modularity, density,entropy];
